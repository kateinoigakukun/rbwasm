...
use rbwasm::{build_cruby, Workspace, Toolchain, BuildSource};
use rbwasm_test_support::init_workspace;

fn fakeruby() -> PathBuf {
    PathBuf::from(file!()).parent().unwrap().join("fakeruby").canonicalize().unwrap()
}
...
fn test_build_cruby_cached() {
    ...
    let build_source = BuildSource::Dir {
        path: fakeruby,
    };
